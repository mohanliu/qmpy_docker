...
RUN bash -c 'source activate oqmd && conda install --yes --quiet python=2.7 atlas numpy scipy matplotlib'
...
CMD ["/bin/bash", "-c", "source activate oqmd && python -i -c 'from qmpy import *'"]
